...
def create_log_processing_rule(rule_dict):
    ...
    required_attributes = ['name', 'source',
                           'known_key_path_pattern', 'log_format']
    ...
    if rule_dict['source'] not in AVAILABLE_LOG_SOURCES:
        raise InvalidLogProcessingRuleFile(
            message="Invalid log source on processing rule.")
    ...
    try:
        processing_rule = LogProcessingRule(
            ...
            header_line_prefix=rule_dict['header_line_prefix']
        )
...
def load_rules_from_dir(directory: str) -> dict:
    ...
    for file in log_processing_rule_files:
        try:
            ...
            logger.exception(
                "There was an error creating the log processing rule. Log processing rule is invalid.")
        ...
        for i, processing_rule_dict in enumerate(yaml_iterator):
            try:
                if isinstance(processing_rule_dict, dict):
                    ...
                    raise InvalidLogProcessingRuleFile(
                        file=str(i), message="Invalid log forwarding rule file")
            except (KeyError, InvalidLogProcessingRuleFile):
                logger.exception(
                    "There was an error creating the log processing rule. Log processing rule %s is invalid", str(i))

    except yaml.YAMLError:
        logger.exception(
            "Encountered an error while parsing log-processing-rules. Aborting...")
...
def load_custom_rules_from_aws_appconfig():
    ...
    raw_processing_rules = aws_appconfig_helpers.get_configuration_from_aws_appconfig(
        'log-processing-rules')

    log_processing_rules = load_processing_rules_from_yaml(
        raw_processing_rules['Body'])
...
def load_custom_rules_from_local_file():
    ...
    logger.info(
        "Loading custom log-processing-rules from local file config/log-processing-rules.yaml ...")
    ...
    try:
        ...
        logger.exception("Unable to open local file %s",
                         DEFAULT_CUSTOM_LOG_PROCESSING_RULES_FILE)
        raise
...
def load_custom_rules():
    ...
    if os.environ.get('LOG_FORWARDER_CONFIGURATION_LOCATION') == 'aws-appconfig':
        ...
        if os.path.isfile(DEFAULT_CUSTOM_LOG_PROCESSING_RULES_FILE):
            ...
            loaded_log_processing_rules = load_rules_from_dir(
                log_processing_rules_directory)
...
def lookup_processing_rule(source: str, source_name: str, processing_rules: dict, key_name: str):
    ...
    # is it a generic or custom rule?
        ...
        if not matched_processing_rule:
            logger.warning(
                "Couldn't find a matching aws processing rule for %s. Defaulting to generic ingestion.", key_name)
            return processing_rules['generic']['generic']
