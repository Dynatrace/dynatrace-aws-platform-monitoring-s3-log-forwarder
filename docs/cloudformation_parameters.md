...
|-----------|------|---------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `GrantReadPermissionToBuckets` | CommaDelimitedList | _(empty)_ | Comma-separated list of S3 bucket names to grant the Lambda function `s3:GetObject` access to and create an EventBridge routing rule for (e.g. `my-bucket,another-bucket`). Leave empty and use the [per-bucket stack](../dynatrace-aws-s3-log-forwarder-s3-bucket-configuration.yaml) for prefix filtering or cross-account/region buckets.                                                                                                                                                                                                                                                                     |
...
| `NotificationType` | String | `EventBridge` | The mechanism used to deliver S3 Object Created notifications to the log forwarder's SQS queue. Allowed values: `EventBridge` — S3 events are routed through Amazon EventBridge (requires enabling EventBridge notifications on each bucket); `SNS` — events flow through an SNS topic you manage outside this stack (requires `S3NotificationsSNSTopicArns` to be set and the topic to exist before deployment); `Direct SQS` — S3 sends notifications directly to the SQS queue (same-region buckets only). See [Step 3 in the deployment guide](deployment_guide.md#step-3-choose-your-s3-notification-type). |
| `S3NotificationsSNSTopicArns` | CommaDelimitedList | _(empty)_ | Comma-separated list of existing SNS topic ARNs permitted to deliver S3 Object Created notifications to the SQS queue (e.g. `arn:aws:sns:us-east-1:123456789012:topic-a,arn:aws:sns:us-east-1:123456789012:topic-b`). Required when `NotificationType` is `SNS`. You are responsible for subscribing each topic to the SQS queue after deployment.                                                                                                                                                                                                                                                               |
